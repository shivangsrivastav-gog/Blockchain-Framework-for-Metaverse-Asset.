...
contract Project {
    ...
    function _removeAssetFromOwner(address _owner, uint256 _assetId) private {
        uint256[] storage assets_owned = ownerAssets[_owner];
        for (uint256 i = 0; i < assets_owned.length; i++) {
            if (assets_owned[i] == _assetId) {
                assets_owned[i] = assets_owned[assets_owned.length - 1];
                assets_owned.pop();
                break;
            }
        }
